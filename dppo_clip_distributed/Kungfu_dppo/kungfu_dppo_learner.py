...
class DPPOLearner(object):
    ...
    def a_train(self, s, a, adv, oldpi_prob):
        ...
        for i in range(len(a_gard)):
            a_gard[i] = all_reduce(a_gard[i],op="sum")
        for i in range(len(a_gard)):
            a_gard[i] = a_gard[i]/2
        write_log("Kungfu learner allreduce and average")
    ...
    def c_train(self, dc_r, s):
        ...
        #c_grad = all_reduce(tape.gradient(closs, self.critic.trainable_weights))
        c_grad = tape.gradient(closs, self.critic.trainable_weights)
        for i in range(len(c_grad)):
            c_grad[i] = (all_reduce(c_grad[i],op="sum"))
        for i in range(len(c_grad)):
            c_grad[i] = c_grad[i]/2
        write_log("Kungfu learner allreduce and average")
        return c_grad
            ...
            if len(batch_s) >= batch_length:
                ...
                for s, a, r, d, adv, logp in zip(*batch_data):
                    ...
                    for _ in range(c_update_steps):
                        ...
                        #c_grad_queue.append(self.c_train(r, s))
