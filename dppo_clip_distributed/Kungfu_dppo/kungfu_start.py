...
'''Design multiprocessing of learners'''
def learner_run(rank):
    from dppo_clip_distributed.dppo_learner import DPPOLearner
    l = len(str(n_learner))
    from dppo_learner import DPPOLearner
    net_func = partial(build_network, 'DPPO_CLIP_LEARNER_{}'.format(str(rank).zfill(l)))
    learner = DPPOLearner(net_func)
    learner.run(traj_queue, grad_queue, should_stop_event, should_update_event, update_barrier,
                param_que_list[1:][rank])
...
if __name__ == '__main__':
    ...
    if True:
        for p in process_list:
            p.start()
        '''Launch multiprocessing of learners'''
        #launch_multiprocess(learner_run, n_learner)
