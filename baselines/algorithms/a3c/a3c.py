...
class ACNet(object):
    ...
    def __init__(self, net_list, scope, entropy_beta, globalAC=None):
...
class Worker(object):
    def __init__(self, env, net_list, name, globalAC, train_episodes, max_steps, gamma, update_itr, entropy_beta, render):
        self.name = name
        self.AC = ACNet(net_list, name, entropy_beta, globalAC )
    ...
    def work(self, globalAC):
        ...
        while not COORD.should_stop() and GLOBAL_EP < self.MAX_GLOBAL_EP:
            ...
            ep_r = 0
            epi_step = 0
            while True and epi_step < self.max_steps:
